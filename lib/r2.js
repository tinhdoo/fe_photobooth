...
function signHeaders({ method, key, canonicalQuery = '', payloadHash, contentType, cacheControl }) {
    const cfg = r2Config();
    ...
    // Cache-Control: gửi header không-ký (R2 lưu như metadata, giống Content-Type). URL ảnh là ngẫu
    // nhiên bất biến -> cache dài an toàn: trình duyệt cache ngay, custom domain (CDN) cache ở edge.
...
// cacheControl mặc định 1 năm + immutable: ảnh có URL ngẫu nhiên, nội dung không đổi.
export async function putR2Object(key, body, contentType, cacheControl = 'public, max-age=31536000, immutable') {
    const { url, headers } = signHeaders({ method: 'PUT', key, payloadHash: 'UNSIGNED-PAYLOAD', contentType, cacheControl });
